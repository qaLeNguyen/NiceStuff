...
public class TestDeleteUser {
    ...
    @Test(dataProvider = "'Delete user' from 'User' dropdown menu", dataProviderClass = DataProviderService.class)
    public void deleteUser(String userName) {
        userPage.selectUser(userName);
        userPage.hoverOverUserDropdown();
        userPage.selectDeleteUser();
        Assert.assertTrue(userPage.isConfirmDeleteUserDisplayed(),
                "The dialog 'Delete the selected user(s)' is not displayed");
        logger.info("The dialog 'Delete the selected user(s)' is displayed properly");

        userPage.clickButtonYesToProceedDeleteUser();
        ...
        Assert.assertTrue(userPage.isUserFound(userName),
                "User cannot be found in 'Recycle'. Failed to delete user");
        logger.info("Delete user '{}' successfully", userName);

    }

    @Test(dataProvider = "'Delete multiple user(s)' from 'User' dropdown menu",
            dataProviderClass = DataProviderService.class)
    public void deleteMultipleUsers(String userName1,
                                    String userName2,
                                    String userName3,
                                    String userName4,
                                    String userName5,
        ...
        Assert.assertTrue(userPage.isConfirmDeleteUserDisplayed(),
                "The dialog 'Delete the selected user(s)' is not displayed");
        logger.info("Selected multiple users. The dialog 'Delete the selected user(s)' is displayed properly");
        userPage.clickButtonYesToProceedDeleteUser();
